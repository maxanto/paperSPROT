...
they present multiple chaotic attractors depending on the selected
point in the parameter's space. For the replacement of the S-box in AES \cite{Ahmad2013,Hussain2013}, or even for the development of a new encryption algorithm.
\end{enumerate}
...
The normalized Shannon Entropy applied to two different \emph{PDF}s and the Maximum Lyapunov Exponent along with the mean period's lengths are the quantifiers employed here to estimate the system's properties. The entropies   help us to evaluate the two properties that determine the randomness degree, the equiprobability among all possible values and the statistical independence between consecutive values, while the $MLE$ determines the presence of chaos. 
...
If $Y$ is the time series being analysed $Y=\{y_i,i=1,...N\}$. The obvious \emph{PDF} to characterize $Y$ is the normalized histogram of the $M$ words $Y$; let us call it  $\emph{PDF}_{hist}$.
...
\begin{equation}
p(\pi)~=~\frac{\sharp \{s|s\leq N-D+1;~ (s) \texttt{, has type
...
Although for $n_f \geqslant 13$ (Figures \ref{fig:avvelo}.i to \ref{fig:avvelo}.l) the attractor appears to be smooth, however, if a \emph{zoom in} is done to the figures (Fig. \ref{fig:m_zoom}) it can be seen that there are still  cycles with different periods that coexist in the attractor for $n_f=14$,$17$ and $18$.
...
The $H_{hist}$-$H_{BP}$ plane, shown in Fig. \ref{fig:HBPvsHhist}, allows a quick visualization of the behavior in terms of randomness of the system, in this plane the ``ideal" point in terms of randomness is $(1,1)$. Here, again, the system seems to stabilize for $n_f$ higher than 12. It can be seen that while the $H_{hist}$ stabilizes close to the maximum value ($1$), the $H_{BP}$ tends to $ 0.5$, this value is characteristic of chaotic systems and is due to the structures of their attractors.
